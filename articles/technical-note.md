...
cell-type motifs (nodes, edges, triangles, and optional wedges), and
tests for differential abundance using edgeR with graph-aware offsets.
...
top_triplets(cellgraph, strategy = "topology", coef = "conditiontreated")
```

## Methodology (volume, co-occurrence, topology)

This section is a technical note aimed at readers who want to understand
...
Motifs are counted on the pruned graph and stored by layer:
...
The volume approach uses label-specific “volumes” to represent how much
of the graph is available for each label in each sample. Volumes are
computed from node degrees in the Delaunay graph: for each label, sum
the degrees of nodes with that label. Let `2m` be twice the number of
edges in the sample.

Offsets are the log expected counts under a Chung-Lu style random graph
that preserves these volumes:

- Nodes: `log(total cells)` with optional TMM correction on the node
  layer.
...
For each motif, counts are modeled with a negative binomial GLM (edgeR
QL) with the volume offset. The design matrix comes from
`design_formula` and can include multiple covariates. The resulting
...
### Co-occurrence triplets (merged)

The co-occurrence strategy applies:

- Volume offsets for 3-node motifs with `triplet_mode = "merge"` to
  collapse wedges + triangles into unordered triplets.

Use `top_triplets(..., strategy = "cooccurrence")` after running
`motif_edger(..., triplet_mode = "merge")`.

### Topology triplets (closure)

Topology modeling uses the submotif-adjusted strategy with
`triplet_mode = "closure"`. Triangles are adjusted by total triples
(open+closed) and wedges by edge-derived covariates (edge-force), while
retaining volume offsets as the baseline exposure. Use
`top_triplets(..., strategy = "topology")` to extract these results.
...
- Samples are independent, and motif counts follow a negative binomial
  model.
- The volume offsets approximate expected counts under a graph with
  preserved label volumes (degree sums) and total edge count.
...
- cellgraph: the main object produced by
...
- Motif: a node, edge, triangle, or wedge defined by labels and counted
  per sample.
- Offset: a log expected count used in the GLM to normalize for
  exposure.
- Strategy: co-occurrence (`volume`, merge) or topology
  (submotif-adjusted, closure).

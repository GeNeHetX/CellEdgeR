...
#' @param motif_key Character motif identifier (e.g., `"A|B|C"`).
...
#' @param motif_key Optional motif identifier (e.g., `"A|B|C"`); when provided, the labels in the key
...
plot_sample_graph <- function(graph_obj, sample_id, max_edge_len = Inf, highlight_labels = NULL,
  ...
  if (is.null(ps$xy)) {
    stop("graph_obj does not store coordinates; rebuild graphs with the current version of build_cell_graphs().")
  }
  xy <- ps$xy
  ...
  if (!is.null(motif_key)) {
    motif_labels <- strsplit(motif_key, "\\|")[[1]]
    if (motif_layer == "size2") {
      motif_pairs <- paste(sort(motif_labels[seq_len(min(2, length(motif_labels)))]), collapse = "|")
    }
  ...
  if (length(edges) && !is.null(motif_labels)) {
    ...
    if (motif_layer == "size2" && !is.null(motif_pairs)) {
      pairs <- ifelse(la <= lb, paste(la, lb, sep = "|"), paste(lb, la, sep = "|"))

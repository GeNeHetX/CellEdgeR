...
(edges), `T_` (triangles), `W_` (wedge), and `TP_` when triangles/wedges
are merged into unordered triplets.
...
1.  **Prepare your data** Provide a *named* list of samples. Each
    ...
    the third column. Optionally add a logical `boundary`/`is_boundary`
    column to flag boundary cells for erosion (excluded by default).
...
- If you want erosion, provide a boundary mask per sample (column
  `boundary`/`is_boundary` or `erosion_cells`); motifs touching those
  cells are excluded without re-triangulating.

...
Use `top_motifs_simple()` to extract node/edge results from this model.
...
- An ANCOVA-style per-motif model for 3-node motifs (triangles/wedges or merged triplets), using volume
  ...
  same baseline exposure. In `triplet_mode = "closure"`, triangles are adjusted
  by total triples (open+closed), while wedges use edge-derived covariates.

Hybrid results can be reconstructed by combining `top_motifs_simple()` (volume)
with `top_motifs_triplet(..., strategy = "ancova")`. FDR is computed separately
for node/edge motifs and 3-node motifs.
...
- Strategy: `volume` or `ancova` modeling choice used in motif retrieval.
